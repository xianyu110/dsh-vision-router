...
export function structuredBootstrapQuestion() {
  return (
    ...
    '"mixed_of":["document|ui|code|chat|general"]（仅当 visual_kind=mixed 时填写，1-2 项，图片混合了哪几种媒介类型）, ' +
    ...
    'mixed_of: ONLY when visual_kind is "mixed", list 1-2 of the media kinds the image mixes (document/ui/code/chat/general); ' +
...
export function normalizeStructuredBootstrapResult(parsed, raw = '') {
  ...
  // content_kind：内容主体类（general 图的大小类判定键）。枚举校验，非法/缺失 → unknown。
  // 与 visual_kind（媒介类）正交；media-kind 图（document/ui/code/chat）约定 unknown。
  const CONTENT_KINDS = new Set(['person', 'animal', 'plant', 'food', 'vehicle', 'machine', 'architecture', 'object', 'scene', 'meme'])
  const contentKind = CONTENT_KINDS.has(textOf(source.content_kind)) ? textOf(source.content_kind) : 'unknown'
  // mixed_of：混合构成（仅 visual_kind=mixed 时有意义）。枚举校验 + 去重 + ≤2，
  // 非法/缺失 → []（调用方 fallback 放行）。与 content_kind 同一"schema 免费收敛"哲学。
  const MIXABLE_KINDS = new Set(['document', 'ui', 'code', 'chat', 'general'])
  const mixedOf = []
  if (Array.isArray(source.mixed_of)) {
    for (const item of source.mixed_of) {
      const kind = textOf(item)
      if (MIXABLE_KINDS.has(kind) && !mixedOf.includes(kind)) mixedOf.push(kind)
      if (mixedOf.length >= 2) break
    }
  }
