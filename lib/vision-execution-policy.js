  ...
  return new Proxy(settings, {
    get(target, property) {
      ...
      return (namespace, ...args) => {
        ...
        const blocked = [...visionExecutionScope.getStore()?.byProvider.entries() ?? []]
...
function llmWithExecutionPolicy(llm) {
      ...
      if (property === 'stream') {
        ...
        return (options) => {
          ...
          if (scope === undefined || !isNonEmptyString(provider) || !isNonEmptyString(model)) {
            return stream.call(target, options)
          }
              ...
              try {
                for await (const chunk of iterable) {
                  ...
                  if (failure !== undefined) {
                    ...
                    setPolicy(scope, provider, model, bridgeDispositionForFailure(failure))
                  }
                  yield chunk
                }
              } catch (error) {
 ...
 * preserves the intended no-YAML compatibility bridge.
 */
export function contextWithVisionExecutionPolicy(ctx) {
  if (!ctx || typeof ctx !== 'object') return ctx
  const cached = wrappedContexts.get(ctx)
  if (cached) return cached

  const llm = llmWithExecutionPolicy(ctx.llm)
